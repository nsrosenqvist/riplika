#!/usr/bin/env python3
# Vendored from flatpak/flatpak-builder-tools (MIT), unchanged.
#
# It turns Cargo.lock into the source list the manifest needs, because a
# flatpak build has no network and every crate has to be named and checksummed
# up front. Kept here rather than fetched during a build so that a build is
# reproducible and so it can be run with no network of its own.
#
# Regenerate with: packaging/regenerate-cargo-sources.sh
# /// script
# requires-python = ">=3.9"
# dependencies = [
#    "aiohttp<4.0.0,>=3.9.5",

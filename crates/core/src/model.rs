...
impl Container {
    /// Everything that differs between formats lives behind [`Container::format`].
    /// These stay because callers read better for them, and delegate so there
    /// is still only one answer to each question.
    pub fn extension(self) -> &'static str {
        self.format().extension()
    }

    pub fn text_subtitle_codec(self) -> &'static str {
        self.format().text_subtitle_codec()
    }

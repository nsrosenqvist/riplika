...
core/src/
  ...
  edc.rs         checking a data sector against the check written inside it
  ...
  art.rs         cover pictures, fetched once and kept
```
...
rather than inside it, since searching by title and returning episodes by season are
...
Almost every bug in this project's history has the same shape: **it produced a result that looked correct.** Not a crash and not an error, but a file that plays, a scan that succeeds, a disc that appears to have no episodes on it.
...
Comments explain **why**, and are worth writing when the reason is not recoverable from the code, such as a constant chosen from a measurement, an ordering that matters, a fallback that exists because of a specific disc. Do not paraphrase the code.

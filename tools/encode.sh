...
#   IVTC     applied only when the source is telecined film
#   audio    passed through, keeping the original AC3 5.1
#   subs     every track carried over
#   faststart moov atom at the front, HandBrake's "web optimized"
#
# Usage: encode.sh <input> <output> [crf] [preset]
set -euo pipefail
IN="$1"; OUT="$2"; CRF="${3:-20}"; PRESET="${4:-medium}"

...
1. **Fetch what is published**, and put back the empty directories it could not send. The new commit goes onto the same ref every release did, and a repository built from scratch each time would drop the history that `flatpak update` walks. S3 holds keys rather than directories, so what comes back has no `refs/remotes`, `refs/mirrors`, `state` or `tmp` - all empty, none created on demand, and their absence stops `build-update-repo` with `opendir(refs/remotes): No such file or directory`, which reads like a corrupt repository and is not one.
